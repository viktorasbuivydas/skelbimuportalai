<x-layouts.app title="Naujas skelbimas">
    <div class="mb-6">
        <a href="{{ route('dashboard.listings.index') }}" class="text-sm text-gray-500 hover:text-gray-700">&larr; Grįžti į skelbimus</a>
        <h1 class="mt-2 text-2xl font-bold text-gray-900">Naujas skelbimas</h1>
    </div>

    <div class="rounded-xl border border-gray-200 bg-white p-6">
        <form method="POST" action="{{ route('dashboard.listings.store') }}" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea
                    ...
                    placeholder="Detalus skelbimo aprašymas..."
                        ...
                        @foreach($metaFields as $key => $field)
                            <div>
                                ...
                                @if($field['type'] === 'boolean')
                                    ...
                                    >
                                        <option value="">—</option>
                                        <option value="1" @selected(old("meta.{$key}") === '1')>Taip</option>
                                        <option value="0" @selected(old("meta.{$key}") === '0')>Ne</option>
                                    </select>
                                @else
                                    <input
