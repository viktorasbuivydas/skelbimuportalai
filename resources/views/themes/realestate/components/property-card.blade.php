...
<article class="group overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
    <div class="relative aspect-[4/3] overflow-hidden">
        ...
        <div class="absolute left-3 top-3 flex flex-wrap gap-2">
            ...
            @endif
            @if($badge)
                <span class="rounded-full bg-emerald-600 px-3 py-1 text-xs font-semibold text-white shadow-sm">{{ $badge }}</span>
            @endif
        </div>
    ...
    <div class="p-4">
        ...
        <h3 class="line-clamp-2 text-base font-semibold text-gray-900 transition group-hover:text-emerald-600">
            {{ $title }}
        </h3>
        ...
        <div class="mt-4 flex items-center justify-between border-t border-gray-100 pt-4">
            <div>
                <span class="text-lg font-bold text-emerald-600">{{ number_format($price, 0, ',', ' ') }} &euro;</span>
            </div>
            <x-button variant="ghost" size="xs" class="text-emerald-600 hover:text-emerald-700!">
                Plačiau
